          ...
          <div class="prizeLise" v-else>
            ...
            <ul>
              <li v-for="(item,index) in prizeList" :key="index">
                ...
                <span>{{timeTrans(item.insertTime)}}</span>
...
export default {
    ...
    this.$nextTick(() => {
      if (this.isApp) {
        ...
        this.wx.miniProgram.postMessage({
          data: {
            title: "",
            // &tokenType=ssoToken&targetSourceId=001005&loginType=ssoToken
  ...
  methods: {
    // 显示活动规则
    ruleClick(){
      this.logPoint("", this.pointName + "_rule", this.sourceid, this.pointName);
      this.rulePop = {
        show: true,
        closeClick: () => this.rulePop.show = false
      }
    ...
    prizeClick(){
      ...
      this.logPoint(this.$store.state.loginServerNumber, this.pointName + "_prize", this.sourceid, this.pointName);
    ...
    showcommonPop(type){
      ...
      if (type == 0) {
        ...
        this.commonPop.text = '来晚一步~活动已经结束啦！'
      } else if (type == 1) {
       ...
       commonService.receiveDetails({
         ...
         if (data.code == 0) {
          this.commonPop = {
            ...
            title: "奖品详情",
            textShow: true,
            text: `<div class="details"><p><span>获得时间：</span>${this.timetrans(item.insertTime)}</p><p><span>活动名称：</span>${item.marketname}</p><p><span>活动奖品：</span>${res.result.prizename}</p><p><span>活动详情：</span></br>${res.result.memo}</p></div>`,
            btnShow: true,
            btnText: "我知道了",
            btnClick: () => this.commonPop.show = false,
          }
