 ...
 * 所以我们可以从这两方面入手，对处罚权限的源头进行控制，最终要实现的目标是：
 *      1.路由方面，用户登录后只能看到自己有访问的导航菜单，也只能访问自己有权访问的路由地址，否则将跳转 4** 提示页。
 *      2.试图方面，用户只能看到自己有权浏览的内容和有权操作的控件。
 ...
 * 登录完拿到 token，将 token 存起来，通过 axios 请求拦截器进行拦截，每次请求到时候头部携带 token
 ...
 * 路有权限控制
 * 方案一
 * 初始化即挂在全部路由，并且在路由上标记相应的权限信息，每次路由跳转前做校验
 * const routerMap = [
 *  {
 *      path: '/permission',
 *      component: Layout,
 ...
 *      3.菜单信息写死在前端，要改个显示文字或者权限信息，都需要重新编译。
 *      4.菜单跟路由耦合在一起，定义路由的时候还要添加菜单显示标题，图标之类的信息，
 *      而且路由不一定作为菜单显示，还要多加字段进行标识。
 *
 * 方案二
 * 初始化的时候先挂挂载不需要权限控制的路由，比如登录页、404错误页等。如果用户通过 URL 进行强行访问，
 * 否则会进入 404，相当于从源头做了控制。
 * 登录后，获取用户的权限信息，然后筛选有权限访问的路由，在全局路由守卫里进行调用 addRouters 添加路由
 ...
 * })
 *
 * 按需挂载，路由就需要知道用户的路由权限，也就是在用户登录进行来的时候就要知道当前用户拥有哪些路由权限
 * 这种方式也存在以下的缺点：
 *      1.全局路由守卫里，每次路由跳转都要做判断
 *      2.菜单信息写死在前端，要改个显示文字或权限信息，都需要重新编译
