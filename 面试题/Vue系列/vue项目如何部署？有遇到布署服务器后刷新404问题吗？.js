 ...
 * 我们先还原一下场景：
 *      1.vue 项目在本地时运行正常，但部署到服务器中，刷新页面，出现了 404 错误。
 * 先定位一下，HTTP 404 错误意味着链接指向的资源不存在。
 * 问题在于为什么不存在？且为什么只有 history 模式下会出现这个问题？
 *
 * 为什么 history 模式下有问题
 * vue 是属于单页应用（single-page application）SPA
 * 而且 SPA 是一种网络应用程序或网站的模型，所有用户交互是通过动态重写当前页面，前面我们也看到了，不管我们应用有多少页面，
 ...
 *
 * 为什么 hash 模式下没有问题
 * router hash 模式我们都知道使用符号 # 表示的，如 website.com/#/login，hash 的值为 #/login
