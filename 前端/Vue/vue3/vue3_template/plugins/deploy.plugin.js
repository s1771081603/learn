const debug = require('debug')('deploy-plugin');

function deployPlugin(modules) {
...
deployPlugin.prototype.apply = (compiler) => {
    const _self = this;
    _self.compiler = compiler;

    compiler.hooks.emit.tap('done', (context) => {
        fs.readFile(path.join(__dirname, '../username.cert'), 'utf8', (err, data) => {
            if (err) {
                console.log(chalk.red(chalk.hex("#a54af7").bold('***************** 无登录用户名，使用默认用户 ******************')))
                init();
                return
            }
            loginData.username = data
            console.log(chalk.red(` 当前使用 ${chalk.hex("#a54af7").bold(`${loginData.username}`)} 账号进行部署操作`))
            init();
        })
