 ...
 * 现在有一个场景，点击新增与编辑的弹框出来进行填写，功能上大同小异，可能只是标题内容或者是显示的主体内容稍微不同。
 * 这时候就没必要写两个组件，只需要根据传入的参数不同，组件显示不同内容即可
 * 这样，下次开发相同界面程序时就可以写更少的代码，意味着更好的开发效率，更少的 BUG 和更少的程序体积
 * */
 ...
 * 除了通过引用 import 的形式，我们还可通过 API 的形式进行组件的调用
 * 还可以包括配置全局样式、国际化与 typescript 结合
 * */
 ...
 * 并且从 body 结构上来看，把 modal 该有的内容（遮罩层、标题、内容、底部按钮）都实现了
 * 关于主体内容
 * <div class="modal__content">
 ...
 * 在 vue2 中，我们可以借助于 vue 实例以及 vue.extend 的方式获得组件实例，然后挂载到 body 上
 ...
 *
 * 事件处理
 * 下面再看看 Modal 组件内部是如何处理 【确定】【取消】 事件的，既然手 vue3，当然采用 composition API 形式
 * // Modal.vue
 * setup(props, ctx) {
