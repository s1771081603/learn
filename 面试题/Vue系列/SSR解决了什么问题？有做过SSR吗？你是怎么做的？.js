/**一、SSR 是什么
 * Server Side Rendering 我们称之为 SSR，意思是服务端渲染。
 ...
 *
 * 单页应用 SPA 单页应用优秀的用户体验，使其逐渐成为主流，页面内容由 JS 渲染出来的，这种方式称为客户端渲染
 * 打开页面查看源码，浏览器拿到的仅有宿主元素，并没有内容
 *
 * 服务端渲染 SSR
 * SSR 解决方案，后端渲染出完整的首屏 dom 结构返回，前段拿到的内容包括首屏及完整 spa 结构，应用激活后依旧按照 spa 方式运行。
 *
 * 看完前端发展，我们再看看 vue 官方对 SSR 的解释：
 * vue.js 是构建客户端应用程序的框架。默认情况下，可以在浏览器中输出 Vue 组件，进行生成 DOM 和操作 DOM。
 * 然而也可以将同一个组件渲染为服务端的 HTML 字符串，将它们直接发送到浏览器，最后将这些静态标记“激活”为客户端上完全可交互的应用程序。
 *
 * 服务单渲染的 vue.js 应用程序可以被认为是“同构”或“通用”，因为应用程序的大部分代码都可以在服务端和客户端运行。
 *
 * 我们从上面解释得到以下结论：
 *      1.vue SSR 是一个在 SPA 上进行改良的服务端渲染
 *      2.通过 vue SSR 渲染的页面，需要在客户端激活才能实现交互
 *      3.vue SSR将包含两部分：服务端渲染的首屏，包含交互的 SPA
 * */

/**二、SSR 解决了什么
 * SSR主要解决了一下两种问题：
 *      1.SEO：搜索引擎优先爬取页面 HTML 结构，使用 SSR时，服务端已经生成了和业务相关联的 HTML，有利于 SEO。
 *      2.首屏呈现渲染：用户无语等待页面所有 js 加载完成就可以看到页面视图（压力来到了服务器，所以需要权衡哪些服务端渲染，哪些交给客户端）
 *
 * 使用 SSR 同样存在以下缺点：
 *      1.复杂度：整个项目的复杂度
 *      2.库的支持性，代码兼容
 *      3.性能问题
 *          1.每个请求都是 n 个实例的创建，不然会污染，消耗会变得很大。
 *          2.缓存 node serve、nginx 判断当前用户有没有过期，如果没有过期的话就缓存，用刚刚的结果
 *          3.降级 监控 cpu、内存占用过多，就 spa 返回单个的壳
 *      4.服务器负载变大，相对于前后端分离，服务器只需要提供静态资源来说，服务器负载更大，所以要慎重使用
 *
 * 所以我们在选择是否使用 SSR 前，我们需要慎重问问自己这些问题：
 *      1.需要 SEO 的页面是否只是少数几个，这些是否可以使用预渲染（prerender SPA Plugin）实现
 *      2.首屏的请求响应逻辑是否复杂，数据返回是否大量且缓慢。
 * */

/**三、如何实现 SSR
 * 对于同购开发，我们依然使用 webpack 打包，我们要解决两个问题：服务端渲染和客户端激活
 * 这里需要生成服务端 bundle 文件用于服务端首屏渲染和客户端 bundle 文件用于客户端激活
 * 代码结构 除了两个不同入口文件之外，其他结构和之前 vue 应用完全相同
 * src
 * ├── router
 * ├────── index.js # 路由声明
 * ├── store
 ...
 *      2.实现 SSR 需要实现服务端首屏渲染和客户端激活
 *      3.服务端异步获取数据 asyncData 可以分为首屏异步获取和切换组件获取
